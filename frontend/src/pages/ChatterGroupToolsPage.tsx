...
// Supported social platforms — must stay in sync with ALLOWED_SOCIAL_PLATFORMS
...
// Click-to-copy social tile. Shows a check mark briefly after copying.
const SocialCopyButton = ({
  platform,
  url,
}: {
  platform: string;
  ...
  const handleCopy = async () => {
    try {
      if (navigator.clipboard?.writeText) {
        ...
        el.select();
        // Deliberate legacy fallback for insecure contexts where `navigator.clipboard` is absent.
        // NOSONAR: execCommand is deprecated but still widely supported and required for HTTP pages.
